...
export default function ComprasPageClient() {
  ...
  useEffect(() => {
    ...
    if (!hasActiveTodayQuotation) {
      // Add virtual "nova cotacao" for today at the end
      ...
      setNavigableQuotations([...allQuotations, virtualNewQuotation]);
    } else {
  ...
  useEffect(() => {
    ...
    if (selectedDate) {
      ...
      if (allClosed || filtered.length === 0) {
        ...
        setFilteredQuotations([...filtered, virtualNewQuotation]);
      } else {
  ...
  useEffect(() => {
    ...
    if (selectedQuotationId) {
      ...
      if (selectedQuotationId === 'nova-cotacao') {
        console.log('[ComprasPageClient] Virtual nova-cotacao selected');
        setListIdForQuotation(null);
        setListDateForQuotation(selectedDate || new Date());
        setQuotationStatus(null);
        setHasActiveQuotation(false);
        previousStatusRef.current = null;
        return;
        ...
        >
          ...
          <TabsContent value="criar-editar" className="mt-8 fade-in" role="tabpanel" aria-labelledby="criar-editar-tab">
            {(() => {
              ...
              if (dateToPass && !listDateForQuotation && !selectedQuotationId) {
                const today = new Date();
                const todayStr = format(today, 'yyyy-MM-dd');
                ...
                if (dateToPassStr === todayStr) {
                  ...
                  if (hasClosedQuotationsToday) {
                    ...
                    dateToPass = tomorrow;
                    console.log('[ComprasPageClient] Replacing today with tomorrow - today has only closed quotations');
                  }
                }
              }
          ...
          <TabsContent value="resultado-envio" className="mt-8 fade-in" role="tabpanel" aria-labelledby="resultado-envio-tab">
            <ResultadoEnvioWrapper selectedQuotationId={selectedQuotationId} />
          </TabsContent>
